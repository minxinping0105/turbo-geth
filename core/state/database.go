...
func (tds *TrieDbState) UnwindTo(blockNr uint64) error {
	...
	for key, value := range storageMap {
		...
		if len(value) > 0 {
			m[keyHash] = value
			if err := tds.db.Put(dbutils.HashedStorageBucket, []byte(key)[:common.AddressLength+common.IncarnationLength+common.HashLength], value); err != nil {
				return err
			}
		} else {
			m[keyHash] = nil
			if err := tds.db.Delete(dbutils.HashedStorageBucket, []byte(key)[:common.AddressLength+common.IncarnationLength+common.HashLength], nil); err != nil {
				return err
			}
		}
	}
